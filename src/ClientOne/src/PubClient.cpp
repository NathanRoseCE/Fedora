...
std::thread create_agent() {
  std::cout << "Creating new agent" << std::endl;
  return std::thread(createAgent);
}
...
//extern "C" {
  ...
  int main(int args,
    ...
    auto future = std::async(std::launch::async, &std::thread::join, &agent_thread);
    if (future.wait_for(std::chrono::seconds(5)) == std::future_status::timeout) { 
      /* --- Do something, if thread has not terminated within 5 s. --- */
      // Delete resources
      uxr_delete_session(&session);
      uxr_close_udp_transport(&transport);
      printf("Forcing shutdown");//TODO not shutting down as intended
      return 0;
